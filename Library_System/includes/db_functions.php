...
function create_user($username, $password, $user_level, $std_id = null, $admin_id = null) {
    ...
    $is_approved = 0; // Require approval for all new users including students and admins
...
function reject_user($account_id) {
    global $conn;
    
    // Option 1: Delete the user record
    $stmt = $conn->prepare("DELETE FROM login_tbl WHERE Account_ID = ?");
    $stmt->bind_param("i", $account_id);
    
    return $stmt->execute();
}

function update_user_credentials($account_id, $username, $password = null) {
    global $conn;

    if ($password !== null) {
        $hashed_password = hash_password($password);
        $stmt = $conn->prepare("UPDATE login_tbl SET username = ?, password = ? WHERE Account_ID = ?");
        $stmt->bind_param("ssi", $username, $hashed_password, $account_id);
    } else {
        $stmt = $conn->prepare("UPDATE login_tbl SET username = ? WHERE Account_ID = ?");
...
function get_total_users() {
    global $conn;
    $result = $conn->query("SELECT COUNT(*) as count FROM login_tbl where is_approved = 1");
    return $result->fetch_assoc()['count'];
}

function get_total_students() {
    global $conn;
    $result = $conn->query("SELECT COUNT(*) as count FROM login_tbl WHERE user_level = 'student' AND is_approved = 1");
    return $result->fetch_assoc()['count'];
}

function get_total_librarians() {
    global $conn;
    $result = $conn->query("SELECT COUNT(*) as count FROM login_tbl WHERE user_level = 'librarian' AND is_approved = 1");
    return $result->fetch_assoc()['count'];
}
